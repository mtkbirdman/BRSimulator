...
public class AerodynamicCalculator : MonoBehaviour
{
    ...
    void InputSpecifications()
    {
        ...
        }else if(MyGameManeger.instance.PlaneName == "Gaillardia"){
            // Plane
            PlaneRigidbody.mass = 100.000f;
            PlaneRigidbody.centerOfMass = new Vector3(0f,-0.203f,0f);
            PlaneRigidbody.inertiaTensor = new Vector3(1396.421f,1441.085f,51.491f); //Ixx, Izz, Iyy
            PlaneRigidbody.inertiaTensorRotation = Quaternion.AngleAxis(-8.519f, Vector3.forward);
            // Specification At Cruise without Ground Effect
